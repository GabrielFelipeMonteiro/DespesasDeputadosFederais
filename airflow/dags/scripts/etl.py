...
from scripts.connDb import path_db
...
def loadDeputadosFinal(df:pl.DataFrame) -> None:
    ...
    """
    try:
        df.write_database("deputados", path_db, if_exists="append", engine="sqlalchemy")
...
def transformDespesas(**context) -> None:
    ...
    temp_file_name = context['task_instance'].xcom_pull(task_ids='task_get_despesas_stg', key='getDespesaStg')
...
def loadDespesasFinal(**context) -> None:
    ...
    temp_file_name = context['task_instance'].xcom_pull(task_ids='task_transform_despesas', key='transformDespesas')

    df = pl.read_csv(temp_file_name)
